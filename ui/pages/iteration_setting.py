...
def replace_input_files_spin(script, epoch):
    # 匹配 --input-files 后的两个参数
    pattern = r'(--input-files\s+)(\S+)(\s+)(\S+)'
    match = re.search(pattern, script)
...
try:
    for i in range(epoch):
        ...
        if os.getenv('syn_method') == "MAGPIE":
            step1_1 = appending(step1, "output_folder", "data_gen")
            step1_1 = appending(step1_1, "output_folder", subdir)
        ...
        if os.getenv('method') == "RLCD":
            ...
            step2_2 = appending(step2, "output-dir", "data_gen")
            step2_2 = modify_lines(step2_2, "output-dir", subdir)
            step2_2 = appending(step2_2, "output-dir", "rejected_dpo")

            step2_2 = replace_input_files_spin(step2_2, i+1)
