...
class Encoder(object):
    ...
    def encode(self, json_line):
        ...
        def process_dialogue(responses):
            doc_ids = [Encoder.tokenizer.sep_token_id] 
            for prompt, response in zip(prompts, responses):
                try:
                    prompt_ids = Encoder.tokenizer.tokenizer(prompt, add_special_tokens=False)['input_ids']
                    response_ids = Encoder.tokenizer.tokenizer(response, add_special_tokens=False)['input_ids']
                    doc_ids.extend(prompt_ids + [Encoder.tokenizer.sep_token_id] + response_ids + [Encoder.tokenizer.sep_token_id])
                except Exception as e:
                    print(f"Error tokenizing: {e}")
                    continue
            
            doc_ids.append(Encoder.tokenizer.eod)  # Add EOD token at the end of the entire dialogue
            
            # Padding
            if self.seq_length > len(doc_ids):
                doc_ids = doc_ids + [Encoder.tokenizer.pad_token_id] * (self.seq_length - len(doc_ids))
            elif self.seq_length < len(doc_ids):
                doc_ids = doc_ids[:self.seq_length]
            
            return doc_ids, [len(doc_ids)]  # Return a single length for the entire dialogue


        chosen_ids, chosen_lens = process_dialogue(chosen)
        rejected_ids, rejected_lens = process_dialogue(rejected)
...
class Partition(object):
    ...
    def process_json_file(self, file_name):
        ...
        for i, (ids, lens, bytes_processed) in enumerate(encoded_docs, start=1):
            total_bytes_processed += bytes_processed
            for key in ['chosen', 'rejected']:
               builders[key].add_document(ids[key], lens[key])
        ...
        print(f"Finish! Created four files!")
        for key in ['chosen', 'rejected']:
            print(f"{output_bin_files[key]} 和 {output_idx_files[key]}")
