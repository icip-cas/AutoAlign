...
class MMapIndexedDatasetBuilder(object):
    def __init__(self, out_file, dtype=np.int64):
        self._data_file = open(out_file, 'wb')  # 即binary write out-file的句柄；
        self._dtype = dtype  # 即'numpy.uint16'
        self._sizes = []  # 存放的是每个句子中word piece的个数；
        self._doc_idx = [0]  # 主动增加了一个0，之后是每个文档中的句子的个数。
