...
if __name__ == "__main__":
    ...
    datas_prompt_tokenizer = []
    for data in datas:
        pos_system, neg_system = format_chat(data["principles"])
        data["p_system"] = pos_system
        data["n_system"] = neg_system
        data["system"] = ""
        datas_pos_prompt_tokenizer.append(format_chat_template(tokenizer, data["prompt"], data["p_system"]))
        datas_neg_prompt_tokenizer.append(format_chat_template(tokenizer, data["prompt"], data["n_system"]))
        datas_prompt_tokenizer.append(format_chat_template(tokenizer, data["prompt"]))
        # Delete the "principles" key from the data dictionary
        del data["principles"]
    ...
    inferencer_raw = MultiProcessVllmInferencer(
        model_path=args.model_path,
        num_gpus_per_model=1,
        do_sample=True,
        num_beams=1,
    ...
    datas_prompt_resposne = inferencer_raw.inference(datas_prompt_tokenizer)
    ...
    for i, data in enumerate(datas):
        ...
        data["raw"] = [{"value":data["prompt"],"from":"human"},{"value":datas_prompt_resposne[i],"from":"gpt"}]
