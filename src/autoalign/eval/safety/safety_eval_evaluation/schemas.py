...
from autoalign.eval.safety.safety_eval_utils.generation_utils import delete_vllm_model, subprocess_inference_with_vllm
from autoalign.eval.safety.safety_eval_utils.generation_utils import (
    load_hf_lm_and_tokenizer,
    generate_completions,
    query_openai_chat_model,
)
from autoalign.eval.safety.safety_eval_utils.templates.single_turn import get_template

from autoalign.conversation import TEMPLATES, Conversation, Role
...
class GeneratorModelBase(abc.ABC):
    def __init__(
        ...
        conversation = Conversation(
            TEMPLATES[self.model_input_template_path_or_name.replace("-keep-system", "")], 
            [(Role.HUMAN, "{instruction}")]
        )
        self.model_input_template = conversation.get_conversation_str(add_generation_prompt=True)
