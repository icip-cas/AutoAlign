...
from autoalign.eval.safety.safety_eval_evaluation.models import load_generative_model
from autoalign.eval.safety.safety_eval_evaluation.schemas import OpenEndedTaskBase, ClassificationTaskBase, JailbreakBase, VllmGeneratorModel
from autoalign.eval.safety.safety_eval_utils.classifier_models.loader import load_classifier_model
from autoalign.eval.safety.safety_eval_evaluation.tasks import load_evaluation_tasks, EvalMode, wrap_task_with_jailbreak
from autoalign.eval.safety.safety_eval_evaluation.utils import maybe_load_evaluation_report, save_evaluation_report
...
def generators(model_name_or_path: str,
    ...
    evaluation_tasks: list[OpenEndedTaskBase] = load_evaluation_tasks(EvalMode.GENERATION, tasks, wildguard_model_path, toxigen_roberta_model_path, llama_guard_model_path)
    evaluation_report = maybe_load_evaluation_report(report_output_path)
    all_individual_results = maybe_load_evaluation_report(save_individual_results_path)
