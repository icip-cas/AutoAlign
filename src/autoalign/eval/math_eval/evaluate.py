...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    if 'idx' in samples[0]:
        samples = {sample['idx']: sample for sample in samples}.values() # 这一步相当于去重
    ...
    col_means= np.array(score_mat).mean(axis=0) # n_samples
    ...
    # 动态添加acc0-accN字段
    for idx, score in enumerate(mean_score):
        result_json[f"acc_sample{idx}"] = score
    result_json["acc"] = np.mean(mean_score)
