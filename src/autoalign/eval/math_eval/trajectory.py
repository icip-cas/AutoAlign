...
def extract_program(text:str=None, trajectory:list=None, last_only=False) -> str:
    ...
    if len(program_list) == 0:
        program_list.append("")
    if len(import_lines) > 0: # 在program_list[0]前面加上import
        program_list[0] = "\n".join(import_lines) + "\n" + program_list[0]
    for i, program in enumerate(program_list[:-1]):
        program_list[i] = "\n".join([line for line in program.split("\n") if not line.strip().startswith("print(")]) # 清除除最后一个程序之外的所有程序中的print

    if last_only:
        program = program_list[-1]
    else:
        program = "\n".join(program_list) # 将所有程序块合并成一个
    return program
