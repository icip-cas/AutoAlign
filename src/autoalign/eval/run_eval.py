...
def handle_result(model_name, opencompass_path):
    ...
    ordered_df = pd.DataFrame(columns=[
        "MT-Bench(EN)",
        "AlpacaEval 1.0(EN)",
        "AlpacaEval 2.0(EN)",
        "AlignBench(CH)",
    ...
    opencompass_outputs_path = os.path.join(opencompass_path,
                                            "outputs/default")
    ...
    for eval in os.listdir(opencompass_outputs_path):
        evaluation_path = os.path.join(opencompass_outputs_path, eval,
                                       "summary")
...
def run_objective_eval(model_name, model_path, eval_type, per_model_gpu,
                       batch_size, opencompass_path, backend):
    if os.path.exists(
            "outputs/{model_name}.csv".format(model_name=model_name)):
            ...
            for file in os.listdir(evaluation_path):
                if "csv" in file:
                    ...
                    if model_name in df.columns:
                        ...
                        break
    config_path = generate_config(model_name, model_path, eval_type,
                                  per_model_gpu, batch_size, opencompass_path,
                                  backend)
    start_opencompass(config_path, opencompass_path)
    ordered_df = handle_result(model_name, opencompass_path)
    if not os.path.isdir("outputs"):
        os.makedirs("outputs")
    ordered_df.to_csv("outputs/{model_name}.csv".format(model_name=model_name),
                      index=False)
